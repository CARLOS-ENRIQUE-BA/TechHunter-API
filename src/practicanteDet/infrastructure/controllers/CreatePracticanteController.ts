...
export class CreatePracticanteDetController {
  constructor(readonly createPracticanteDetUseCase: CreatePracticanteDetUseCase) { }
  async run(req: Request, res: Response) {
    const data = req.body;
    console.log("mensaje  " + data);
    try {
      const practicanteDet = await this.createPracticanteDetUseCase.run(
        data.correo,
        data.preferencias,
